...
namespace JH\Magento\Command\Customer;

use N98\Magento\Command\AbstractMagentoCommand;
use Symfony\Component\Console\Input\InputInterface;
use Symfony\Component\Console\Output\OutputInterface;

class CleanAddressVatIDCommand extends AbstractMagentoCommand
{
    protected function configure()
    {
      $this
          ->setName('customer:clean-address-vat-id')
          ->setDescription('Clean up Vat-ID address attribute (with code \'vat_id\') by stripping spaces and dots.')
      ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($this->initMagento()) {
            ...
            $taxVatAttributeId = $db->fetchOne("SELECT attribute_id FROM {$resource->getTableName('eav/attribute')} 
                WHERE attribute_code = ?", array('vat_id'));
                
            $rows = $db->fetchAll("SELECT * FROM {$resource->getTableName('customer_address_entity_varchar')} 
                WHERE entity_type_id = ? AND attribute_id = ?
                AND `value` IS NOT NULL", array(2, $taxVatAttributeId));
